#!/usr/bin/env Rscript
# ==============================================================================
# [Script]: 2.1.3.1_OneK1K_sceQTL_susie_coloc_blocks_4windows_template.R
# [Method]: SuSiE-based Colocalization 
# [Step]:  ( OneK1K sc-eQTL)
# 
# [Function]:
# Conduct SuSiE-based colocalization analysis to identify shared causal variants
#       between sc-eQTL and GWAS traits. Execute analysis in 4 windows per LD block.
#       Includes smart cell-type aware chunking and LD caching.
#        LD . 
# 
# [Data Availability / ]:
# Requires preprocessed sc-eQTL (with cell subdirs), GWAS, and UKB LD panel.
# ==============================================================================
...
option_list <- list(
  ...
  make_option(c("--eqtl_dir"), type="character", default=NULL, help="Path to preprocessed sc-eQTL directory (with cell subfolders)"),
...
if (!"cell" %in% colnames(gene_info)) stop("Gene info missing 'cell' column")
if (!"gene_id" %in% colnames(gene_info)) stop("Gene info missing 'gene_id' column")
...
gwas_dt <- gwas_dt[!((A1 == "A" & A2 == "T") | (A1 == "T" & A2 == "A") | (A1 == "C" & A2 == "G") | (A1 == "G" & A2 == "C"))]
...
if (!exists("worker_ld_cache")) {
  worker_ld_cache <- new.env(parent = emptyenv())
  worker_ld_cache$data <- list()
  worker_ld_cache$current_block_key <- NULL
}

compute_ld_mem_cached <- function(snps, ld_res, chr, start, end, block_ld_dir) {
  block_path <- find_block_ld_file(chr, start, end, block_ld_dir)
  if (is.null(block_path)) return(NULL)
  
  if (is.null(worker_ld_cache$current_block_key) || worker_ld_cache$current_block_key != block_path) {
      rm(list = ls(envir = worker_ld_cache), envir = worker_ld_cache)
      worker_ld_cache$data <- list()
      worker_ld_cache$current_block_key <- NULL
      gc(verbose=FALSE)
      
      rds_path <- paste0(block_path, ".rds")
      if (!file.exists(rds_path)) {
         bed_path <- paste0(block_path, ".bed")
         if (file.exists(bed_path)) tryCatch(bigsnpr::snp_readBed(bed_path), error=function(e) NULL)
      }
      if (file.exists(rds_path)) {
          worker_ld_cache$data <- bigsnpr::snp_attach(rds_path)
          worker_ld_cache$current_block_key <- block_path
      }
  }
  
  if (is.null(worker_ld_cache$data) || is.null(worker_ld_cache$data$genotypes)) return(NULL)
  
  bigsnp <- worker_ld_cache$data
  map <- bigsnp$map
...
create_locuscompare_plot <- function(eqtl, gwas, gene_id, gene_name, chr, start, end, out_dir, file_prefix, lead_snp=NULL) {
  ...
  tryCatch({
    p <- locuscomparer::locuscompare(in_fn1=fn2, in_fn2=fn1, title1="GWAS", title2=paste0(gene_name, " sc-eQTL"), snp=lead_snp, genome="hg19")
...
execute_window_analysis <- function(tsk, eqtl, ld_res, gwas_data, super_data = NULL) {
  ...
  if (!is.null(super_data)) {
    ...
    eqtl_sub <- eqtl[chr == tsk$chr & pos >= s & pos <= e]
    gwas_sub <- gwas_data[chr == tsk$chr & pos >= s & pos <= e]
    ...
    R <- compute_ld_mem_cached(unique(merged$snp), ld_res, chr, s, e, opt$block_ld_dir)
...
process_gene_batch <- function(gene_task, ld_res, gwas_chr_data) {
  eqtl_file <- file.path(opt$eqtl_dir, gene_task$cell, paste0(gene_task$ens, ".csv"))
  if (!file.exists(eqtl_file)) return(lapply(gene_task$windows, function(w) list(status="missing_eqtl")))
  
  eqtl_dt <- tryCatch(fread(eqtl_file), error=function(e) NULL)
  if (is.null(eqtl_dt)) return(lapply(gene_task$windows, function(w) list(status="read_fail")))
  
  if ("A1" %in% colnames(eqtl_dt) && "A2" %in% colnames(eqtl_dt)) {
      eqtl_dt <- eqtl_dt[!((A1 == "A" & A2 == "T") | (A1 == "T" & A2 == "A") | (A1 == "C" & A2 == "G") | (A1 == "G" & A2 == "C"))]
  }
  
  results <- list()
  block_ids <- unique(sapply(gene_task$windows, function(w) w$block_id))
  
  for (bid in block_ids) {
      curr_windows <- gene_task$windows[sapply(gene_task$windows, function(w) w$block_id) == bid]
      s_min <- min(sapply(curr_windows, function(w) w$start))
      e_max <- max(sapply(curr_windows, function(w) w$end))
      
      super_data <- NULL
      tryCatch({
          eqtl_sup <- eqtl_dt[chr == gene_task$chr & pos >= s_min & pos <= e_max]
          gwas_sup <- gwas_chr_data[chr == gene_task$chr & pos >= s_min & pos <= e_max]
          if ("pval" %in% names(gwas_sup)) gwas_sup <- gwas_sup[order(pval)]
          gwas_sup <- unique(gwas_sup, by = "snp")
          
          if (nrow(eqtl_sup) >= 10 && nrow(gwas_sup) >= 10) {
              merged <- merge(eqtl_sup, gwas_sup, by="snp", suffixes=c("_eqtl", "_gwas"))
              flip <- (merged$A1_eqtl == merged$A2_gwas & merged$A2_eqtl == merged$A1_gwas)
              merged$beta_gwas[flip] <- -merged$beta_gwas[flip]
              merged <- merged[(merged$A1_eqtl == merged$A1_gwas & merged$A2_eqtl == merged$A2_gwas) | flip]
              if (nrow(merged) >= 10) {
                  R <- compute_ld_mem_cached(unique(merged$snp), ld_res, gene_task$chr, s_min, e_max, opt$block_ld_dir)
                  if (!is.null(R)) {
                      common <- intersect(merged$snp, rownames(R))
      ...
      for (win_conf in curr_windows) {
          tsk <- list(ens=gene_task$ens, cell=gene_task$cell, chr=gene_task$chr, start=win_conf$start, end=win_conf$end, block_id=win_conf$block_id, window_tag=win_conf$window_tag)
          res <- tryCatch(execute_window_analysis(tsk, eqtl_dt, ld_res, gwas_chr_data, super_data), error=function(e) list(status="error"))
  ...
  for (i in seq_len(nrow(chr_genes))) {
    ...
    for (b in 1:row$num_ld_blocks) {
      ...
      for (wd in list(list(tag="block", s=bs, e=be), list(tag="100kb", s=max(bs, row$start_hg19-1e5), e=min(be, row$end_hg19+1e5)), list(tag="50kb", s=max(bs, row$start_hg19-5e4), e=min(be, row$end_hg19+5e4)), list(tag="10kb", s=max(bs, row$start_hg19-1e4), e=min(be, row$end_hg19+1e4)))) {
        if (!paste(row$cell, row$ens_gene_id, b, wd$tag, sep="_") %in% completed_tasks) windows_config[[length(windows_config)+1]] <- list(block_id=b, window_tag=wd$tag, start=wd$s, end=wd$e)
      }
    }
    if (length(windows_config) > 0) gene_batches[[length(gene_batches)+1]] <- list(ens=row$ens_gene_id, cell=row$cell, chr=chrom, windows=windows_config, block_key=paste0(chrom,"_",row$block1_start,"_",row$block1_end))
  }
  
  if (length(gene_batches) == 0) next
  
  ld_res <- list(type = "lazy_block", block_ld_dir = opt$block_ld_dir)
  tasks_by_block <- split(gene_batches, sapply(gene_batches, function(x) x$block_key))
  final_chunks <- list()
  
  for (blk_tasks in tasks_by_block) {
    if (length(blk_tasks) > opt$chunk_size) {
      final_chunks <- c(final_chunks, split(blk_tasks, ceiling(seq_along(blk_tasks)/opt$chunk_size)))
    } else {
      final_chunks[[length(final_chunks)+1]] <- blk_tasks
    }
  }
  
  if (opt$test_n > 0) final_chunks <- head(final_chunks, ceiling(opt$test_n / opt$chunk_size))
  
  res_chr <- future_lapply(final_chunks, function(chunk_tasks) {
    ...
    chunk_results <- list()
    for (gb in chunk_tasks) chunk_results <- c(chunk_results, process_gene_batch(gb, ld_res, gwas_chr_subset))
    return(chunk_results)
  }, future.seed=TRUE)
  
  all_stats <- c(all_stats, unlist(res_chr, recursive=FALSE))
  task_count <- task_count + length(gene_batches)
